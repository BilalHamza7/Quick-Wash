...
@RestController
@CrossOrigin("http://localhost:3084")
public class QuickService_Controller
{
    @Autowired
    ...
    @DeleteMapping(path = "/quickServices/{vehicleNum}")                            // when a service is finished it will be inserted in the dequeue_service_tbl
    ...
    @GetMapping(path = "/quickFinishedServices/{id}")                                     // Displaying all Finished services
    public QuickServiceFinished getFinishedServiceById(@PathVariable int id)
    {
        return  quickServiceService.getFinishedServiceById(id);
    }
}
