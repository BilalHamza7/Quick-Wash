...
export default function NewService() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
        const response = await axios.get('http://localhost:8082/quickServices');
  ...
  const onSubmit = async (e)=> {
      ...
      if(existingVehicle) {
        ...
        await axios.post(`http://localhost:8082/quickServices`, {...service, vehicleNumber, serviceName, serviceCost});
        navigate("/service");
      }
  }
